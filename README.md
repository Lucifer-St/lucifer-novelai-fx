...
- **主提示词与角色同栏**：合并布局连续显示主正负面和角色提示词；主区、每个角色可独立折叠。折叠不会停用角色或改变生成参数。
- **保留编辑现场**：进入工具页、切换布局或折叠时保留原文本框、光标、撤销历史和卡片归属；布局偏好独立保存。

两种布局已覆盖全部 8 款 Windows 分享版皮肤，以及桌面、短窗口和手机宽度。简易双栏暂不发布。个人版新增的 Grok 创作工具不包含在分享包中。
